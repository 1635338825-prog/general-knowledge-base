...
如果来源是网页而不是 PDF，可以直接走 Firecrawl 网页接入：
...
- 调用 Firecrawl 抓取页面 Markdown
- 统一写入 `derived/<source-id>/content.md`
- 统一写入 `derived/<source-id>/content.json`
- 更新 `.wiki-cache.json`
- 默认继续生成 Codex digest bundle
...
对于网页来源，可用 `web-ingest` 或 `firecrawl-ingest` 直接进入同一条来源消化链，不需要先手动保存网页内容。
